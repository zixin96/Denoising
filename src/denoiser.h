...
struct FrameInfo {
  public:
    ...
    Buffer2D<float> m_id;
    std::vector<Matrix4x4> m_matrix; // objToWorld for object i 
};
...
class Denoiser {
  public:
    ...
    Buffer2D<Float3> ProcessFrameDenoiseOnly(const FrameInfo &frameInfo);
    Buffer2D<Float3> ProcessFrameReproAccuOnly(const FrameInfo &frameInfo);
